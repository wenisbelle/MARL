...
def main():
    ...
    for step in range(1000):
        ...
        # Overwrite the rows for flagged agents with a custom target.
        #if decision_step:
        #    action = td["agents", "action"]   # tensor of shape (N, 2), shares storage with td
        #    for slot in has_flag.nonzero(as_tuple=True)[0].tolist():
        #        action[slot] = torch.tensor([0.5, 0.5], dtype=action.dtype)
