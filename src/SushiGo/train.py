...
=======================
With independent learning the opponents are themselves learning, so each agent
faces a moving target. Parameter sharing + self-play softens this (all seats move
together). The replay buffer can still hold transitions generated against older
opponent behaviour — keep the buffer moderate in size so it does not get too stale.
...
from sushi_go_env import N_TYPES
from torchrl_integration import make_torchrl_env, OBS_KEY, MASK_KEY, GROUP, ACTION_KEY
...
def train(args):
    ...
    # SoftUpdate is how the target network tracks the live network — every step it does θ_target ← eps·θ_target + (1-eps)·θ_live.
    # With eps=0.995 the target moves 0.5% of the way toward the live network each step: slow, smooth tracking. 
